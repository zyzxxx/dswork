...
/**
 * Single Word Multi Char Query Builder 分词算法专用
 * @author linliangyi
 */
public class SWMCQueryBuilder
{
	...
	private static List<Lexeme> doAnalyze(String keywords)
	{
		...
		BaseSegmenter ikSeg = new BaseSegmenter(new StringReader(keywords), true);
		try
		{
			Lexeme l = null;
	...
	private static Query getSWMCQuery(String fieldName, List<Lexeme> lexemes, boolean quickMode)
	{
		...
		QueryParser qp = new QueryParser(fieldName, new dswork.analyzer.BaseAnalyzer());
