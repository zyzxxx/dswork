 ...
 * 结合SWMCQuery算法
 * 表达式例子 ：
 * (id='1231231' && title:'monkey') || (content:'你好吗' || url='www.ik.com') - name:'helloword'
 * @author linliangyi
 */
public class IKQueryExpressionParser
{
	...
	private Query toBooleanQuery(Element op)
	{
		...
		org.apache.lucene.search.BooleanQuery.Builder resultQuery = new BooleanQuery.Builder();
		// BooleanQuery resultQuery = builder.add(clause);
		...
		if('&' == op.type)
		{
			if(q1 != null)
			{
				resultQuery.add(new BooleanClause(q1, Occur.MUST));
			}
			if(q2 != null)
			{
				resultQuery.add(new BooleanClause(q2, Occur.MUST));
			}
		}
		else if('|' == op.type)
		{
			if(q1 != null)
			{
				resultQuery.add(new BooleanClause(q1, Occur.SHOULD));
			}
			if(q2 != null)
			{
				resultQuery.add(new BooleanClause(q2, Occur.SHOULD));
			}
		}
		else if('-' == op.type)
		{
			...
			resultQuery.add(new BooleanClause(q1, Occur.MUST));
			resultQuery.add(new BooleanClause(q2, Occur.MUST_NOT));
		}
