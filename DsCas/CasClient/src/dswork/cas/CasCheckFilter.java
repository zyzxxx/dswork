...
import dswork.cas.service.CasFactory;
...
public class CasCheckFilter implements Filter
{
	...
	private static Long SYSTEM_REFRESH = EnvironmentUtil.getToLong(KEY_SYSTEM_REFRESH, 60000);// 当前系统的全部权限缓存更新间隔1000(1秒)|60000(1分钟)|3600000(1小时)|86400000(1天)
	private static String SYSTEM_ALIAS = EnvironmentUtil.getToString(KEY_SYSTEM_ALIAS, "");//当前系统的标识
	private static String SYSTEM_PASSWORD = EnvironmentUtil.getToString(KEY_SYSTEM_PASSWORD, "");;//当前系统的标识
	...
	private static void refreshSystemFunc()
	{
		if(System.currentTimeMillis() > refreshTime)// 初始时refreshPermissionTime为0，肯定会执行
		{
			...
			try
			{
				resUrlMap.clear();
				resMap.clear();
				IFunc[] funcArray = CasFactory.getDsworkCasService().getFunctionBySystem(SYSTEM_ALIAS, SYSTEM_PASSWORD);// 初始化该系统已配置的全部功能
	...
	private static Map<String, List<String>> getUserFunc(String userAccount, HttpSession session)
	{
		...
		{
			IFunc[] myFuncArray = CasFactory.getDsworkCasService().getFunctionByUser(SYSTEM_ALIAS, SYSTEM_PASSWORD, userAccount);
	...
	public static IFunc[] getFuncByUser(String account)
	{
		...
		{
			IFunc[] arr = CasFactory.getDsworkCasService().getFunctionByUser(SYSTEM_ALIAS, SYSTEM_PASSWORD, account);
			if(arr != null)
			{
				for(IFunc m : arr)
				{
