...
namespace Dswork.Core.Util
{
	...
	public class EncryptUtil
	{
        ...
        /// <param name="firstKey">第一个密钥</param>
        /// <param name="secondKey">第二个密钥</param>
        /// <param name="thirdKey">第三个密钥</param>
        /// <returns>Des编码的String，失败返回null</returns>
        public static String EncodeDes(String str, String firstKey, String secondKey, String thirdKey)
        {
            int leng = str.Length;
            String encData = "";
            ...
            if (leng > 0)
            {
                ...
                else
                {
                    ...
                    if (remainder > 0)
                    {
                        String remainderData = str.Substring(iterator * 4 + 0);//, leng 与java和js的substring不同，与js的substr方法相同
        ...
        /// <param name="thirdKey">第三个密钥</param>
        /// <returns>解码后的字符串，失败返回null</returns>
        public static String DecodeDes(String str, String firstKey, String secondKey, String thirdKey)
        {
            int leng = str.Length;
            String decStr = "";
            List<int[]> firstKeyBt = null, secondKeyBt = null, thirdKeyBt = null;
            ...
            for (i = 0; i < iterator; i++)
            {
                String tempData = str.Substring(i * 16 + 0, 16);// 与java和js的substring不同，与js的substr方法相同
                String strByte = hexToBt64(tempData);
                int[] intByte = new int[64];
                int j = 0;
                for (j = 0; j < 64; j++)
                {
                    intByte[j] = int.Parse(strByte.Substring(j, 1));// 与java和js的substring不同，与js的substr方法相同
                }
                int[] decByte = null;
                if (firstKey != null && firstKey != "" && secondKey != null && secondKey != "" && thirdKey != null && thirdKey != "")
                {
        ...
        private static String hexToBt64(String hex)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                binary += hexToBt4(hex.Substring(i, 1));// 与java和js的substring不同，与js的substr方法相同
            }
            return binary;
        }
            ...
            int[,] s8 =
		{
            ...
            for (int m = 0; m < 8; m++)
            {
                ...
                sBoxByte[m * 4 + 0] = int.Parse(binary.Substring(0, 1));// 与java和js的substring不同，与js的substr方法相同
                sBoxByte[m * 4 + 1] = int.Parse(binary.Substring(1, 1));// 与java和js的substring不同，与js的substr方法相同
                sBoxByte[m * 4 + 2] = int.Parse(binary.Substring(2, 1));// 与java和js的substring不同，与js的substr方法相同
                sBoxByte[m * 4 + 3] = int.Parse(binary.Substring(3, 1));// 与java和js的substring不同，与js的substr方法相同
            }
            return sBoxByte;
        }
