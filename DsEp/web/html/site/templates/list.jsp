  ...
  <div class="w735 right">
	<div class="listpage hei1">
	  ...
	  <div class="page">
		...
		<a<c:if test="${datapage.page == datapage.last}"> onclick="return false;"</c:if> href="${ctx}/${datapage.lasturl}">首页</a>
		...
		<c:forEach var="i" begin="${datapage.page - 3}" end="${datapage.page + 3}">
			<c:if test="${i > 1 && i < datapage.last}"><a${datapage.page == i?' class="selected"':''} href="${ctx}/${datauri}_${i}.html">${i}</a></c:if>
		</c:forEach>
		<c:if test="${datapage.page + 3 > 2 && datapage.page + 3 < datapage.last - 3}"><a href="${ctx}/${datauri}_${datapage.page + 4}.html">...</a></c:if>
		<a${datapage.page == datapage.last?' class="selected"':''}<c:if test="${datapage.page == datapage.last}"> onclick="return false;"</c:if> href="${ctx}/${datapage.lasturl}">${datapage.last}</a>
	  </div>
	</div>
  </div>
</div>
