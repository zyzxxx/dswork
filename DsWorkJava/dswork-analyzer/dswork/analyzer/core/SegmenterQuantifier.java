 ...
 * 中文数量词子分词器
 */
class SegmenterQuantifier implements Segmenter
{
	// 子分词器标签
	...
	private boolean needCountScan(AnalyzeContext context)
	{
		...
		else
		{
			...
			if(!context.getOrgLexemes().isEmpty())
			{
				...
				if(Lexeme.TYPE_CNNUM == l.getLexemeType() || Lexeme.TYPE_ARABIC == l.getLexemeType())
				{
	...
	private void outputNumLexeme(AnalyzeContext context)
	{
		...
		{
			// 输出数词
			Lexeme newLexeme = new Lexeme(context.getBufferOffset(), nStart, nEnd - nStart + 1, Lexeme.TYPE_CNNUM);
			context.addLexeme(newLexeme);
		}
	}
}
