...
//		boolean flag2 = false;
		...
		// System.out.println("totalbytes:" + m_totalBytes);
		...
		for(m_currentIndex++; m_currentIndex < m_totalBytes; m_currentIndex = m_currentIndex + 2)
		{
			...
			if(flag3)
			{
				dswork.core.upload.jspsmart.File file = new dswork.core.upload.jspsmart.File();
				file.setParent(this);
				file.setFieldName(s3);
				if(s5 != null && s4 != null && s5.length() > 0 && s4.length() > 0)
					s4 = s4.substring(0, s4.length() - s5.length()) + s5;
				file.setFileName(s4);
				file.setFileExt(s5);
	...
	public void close()
	{
		...
		{
			m_deniedFilesList.clear();
			m_deniedFilesList = null;
		}
		if(m_allowedFilesList != null)
		{
			m_allowedFilesList.clear();
...
//		boolean flag = false;
		...
		// System.out.println("原来的name：" + m_binArray);
		String s = "";
		try
		{
