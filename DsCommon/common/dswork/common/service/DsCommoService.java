...
public class DsCommoService
{
	...
	public static String start(String alias, String ywlsh, String caccount, String cname, String piDay, boolean isWorkDay, String taskInterface, String userInterface)
	{
		...
		String deployid = dao.queryDeployid(alias);
		Long flowid = dao.queryFlowid(deployid);
		IFlowTask task = dao.queryTask(flowid, "start");
		if(task != null)
		{
			...
			if(piid != null)
			{
				...
				if(s.length > 1)
				{
					pidoing.setThaccount("," + task.getTusers() + ",");
				}
				else
				{
					pidoing.setTaccount("," + task.getTusers() + ",");
				}
				dao.saveFlowPiDoing(pidoing);
			}
	...
	{
		SimpleDateFormat sj = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		IFlowPiDoing pidoing = dao.queryByid(doingid);
		if(pidoing != null)
		{
			...
			pidata.setStatus(0);
			IFlowTask task = dao.queryTask(pidoing.getFlowid(), pidoing.getTalias());
			pidata.setTname(task.getTname());
			dao.saveFlowPiData(pidata);
			dao.deletePiDoing(doingid);
			for(int i = 0; i < nextTalias.length; i++)
			{
				if(dao.isExistsByTalias(pidoing.getPiid(), nextTalias[i]))
				{
					dao.updateTcount(pidoing.getTcount() - 1, pidoing.getFlowid(), pidoing.getTalias());
				}
				else
				{
					IFlowTask t = dao.queryTask(pidoing.getFlowid(), nextTalias[i]);
					...
					if(s.length > 1)
					{
						m.setThaccount("," + t.getTusers() + ",");
					}
					else
					{
						m.setTaccount("," + t.getTusers() + ",");
					}
			...
			List<IFlowPiDoing> list = dao.queryBypiid(pidoing.getPiid());
			if(list.isEmpty())
			{
				dao.updateStatus(pidoing.getPiid());
			}
			else
			{
				String str = "";
				...
				dao.updatePialias(pidoing.getPiid(), str);
			}
			return true;
		}
		else
