...
DROP TABLE IF EXISTS DS_CMS_AUDIT_PAGE;
DROP TABLE IF EXISTS DS_CMS_AUDIT_CATEGORY;
...
(
   ...
   PID                  BIGINT COMMENT '父ID',
   SITEID               BIGINT COMMENT '站点ID',
   STATUS               INT COMMENT '状态(0新增待发,1更新待发,8已发)',
...
CREATE TABLE DS_CMS_AUDIT_CATEGORY
(
   ID                   BIGINT NOT NULL COMMENT '主键',
   SITEID               BIGINT COMMENT '站点ID',
   STATUS               INT COMMENT '状态(0草稿,1未审核,2不通过,4通过)',
   ...
   CONSTRAINT FK_DS_CMS_AUDIT_CATEGORY_SITE FOREIGN KEY (SITEID)
      REFERENCES DS_CMS_SITE (ID) ON DELETE CASCADE ON UPDATE CASCADE,
   CONSTRAINT FK_DS_CMS_AUDIT_CATEGORY_CATEGORY FOREIGN KEY (ID)
      REFERENCES DS_CMS_CATEGORY (ID) ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT '审核栏目';
CREATE TABLE DS_CMS_AUDIT_PAGE
(
   ID                   BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键',
   SITEID               BIGINT COMMENT '站点ID',
   CATEGORYID           BIGINT COMMENT '栏目ID',
   ...
   PAGETOP              INT COMMENT '首页推荐(0否,1是)',
   CONTENT              TEXT COMMENT '内容',
   USERACCOUNT          VARCHAR(200) COMMENT '用户账号',
   PRIMARY KEY (ID),
   CONSTRAINT FK_DS_CMS_AUDIT_PAGE_SITE FOREIGN KEY (SITEID)
      REFERENCES DS_CMS_SITE (ID) ON DELETE CASCADE ON UPDATE CASCADE,
   CONSTRAINT FK_DS_CMS_AUDIT_PAGE_CATEGORY FOREIGN KEY (CATEGORYID)
      REFERENCES DS_CMS_CATEGORY (ID) ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT '审核内容';
